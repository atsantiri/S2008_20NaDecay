...
void plot2DEvtsWithChargeFit()
{
    ...
    d.Foreach(
        [&](ActRoot::TPCData& tpc, std::vector<bool> isDecay)
        {
            ...
            for(auto& c : clusters)
            {
                if((k < kmax) && isDecay[c.GetClusterID()])
                {
                    ...
                    auto hcharge =
                        new TH1D {TString::Format("hCharge%d", k), "Charge deposited; L [mm]; Q", 50, 0, 150};
                    ...
                    auto hCharge2D = new TH2F(TString::Format("hCharge2D%d", k),
                                              "Charge deposited;x [pad]; y [pad]; Q [au]", TPCparams.GetNPADSX(), 0,
                                              TPCparams.GetNPADSX(), TPCparams.GetNPADSY(), 0, TPCparams.GetNPADSY());
